<script lang="ts">
    ...
    let totalPages = $derived(
        Math.max(1, Math.ceil(schools.length / PER_PAGE)),
    );
    ...
    function handlePageChange(page: number) {
        currentPage = page;
        document
            .querySelector(".school-list-wrap")
...
<div class="school-list-wrap">
    ...
    <div class="list-controls">
        <span class="pg-summary-text">
            {#if schools.length > 0}
                Wyświetlono {(effectivePage - 1) * PER_PAGE + 1}–{Math.min(
    ...
    {#if viewMode === "table"}
        <div class="table-card sf-card">
            ...
            {#each paginatedSchools as school, i (school.id)}
                <a
                    href={`${getListingPath(school)}${linkSuffix}`}
                    class="table-row {fadingClass(school)}"
                    style="grid-template-columns: {gridTemplate}"
                >
                    {#each columns as col (col)}
                        {#if col === "rank"}
                            <span class="td td--rank"
                                >{String(
                                    (effectivePage - 1) * PER_PAGE + i + 1,
                                ).padStart(2, "0")}</span
                            >
                        {:else if col === "name"}
                        ...
                        {:else if col === "rating"}
                            <span class="td td--rating">
                                {#if school.rating != null}
                                    <span class="rating"
                                        >{school.rating.toFixed(1)}</span
                                    >
                                    {#if school.reviews != null}
                                        <span class="review-count muted"
                                            >({school.reviews})</span
                                        >
                                    {/if}
                                {:else}
                    ...
                    {#if school.styles.length > 0}
                        <div class="card-styles">
                            ...
                            {#if school.styles.length > 4}
                                <span class="style-tag style-tag--more"
                                    >+{school.styles.length - 4}</span
                                >
                            {/if}
                        </div>
                    {/if}
