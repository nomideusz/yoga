    ...
    function initMap() {
        ...
        for (const c of cities) {
            ...
            const infoWindow = new google.maps.InfoWindow({
                content: `<div style="font-family:sans-serif;padding:4px 2px;color:var(--sf-dark)">
          <strong style="font-size:14px">${c.city}</strong><br>
          <span style="font-size:12px;color:var(--sf-text)">${c.count} ${c.count === 1 ? "szkoła" : c.count <= 4 ? "szkoły" : "szkół"}</span>
        </div>`,
            });
