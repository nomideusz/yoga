<script lang="ts">
  ...
  let placeSuggestions: Array<{ description: string; placeId: string }> = $state([]);
  ...
  type SearchResult =
    ...
    | { type: "school"; id: string; name: string; city: string; rating: number | null }
  ...
  const searchResults = $derived.by((): SearchResult[] => {
    ...
    if (postalFull) {
      results.push({ type: "postal", code: `${postalFull[1]}-${postalFull[2]}` });
      ...
      results.push({ type: "postal", code: digits.length >= 2 ? `${digits.slice(0, 2)}-${digits.slice(2)}` : digits });
    }
    ...
    if (tokens.length >= 2) {
      ...
      for (let i = 0; i < tokens.length - 1; i++) {
        ...
        const cityFirst = allCities.find((c) => normalizePolish(c) === left);
        if (cityFirst) {
          const matchingStyles = allStyles.filter((s) => normalizePolish(s).includes(right));
          for (const style of matchingStyles) {
            const key = `${cityFirst}+${style}`;
            if (!seen.has(key)) {
              const count = listings.filter(
                (l) => l.city === cityFirst && l.styles.includes(style),
              ).length;
              if (count > 0) { seen.add(key); results.push({ type: "city+style", city: cityFirst, style, count }); }
            }
          }
        }

        // Try style-first: left=style(s), right=city
        const citySecond = allCities.find((c) => normalizePolish(c) === right);
        if (citySecond) {
          const matchingStyles = allStyles.filter((s) => normalizePolish(s).includes(left));
          for (const style of matchingStyles) {
            const key = `${citySecond}+${style}`;
            if (!seen.has(key)) {
              const count = listings.filter(
                (l) => l.city === citySecond && l.styles.includes(style),
              ).length;
              if (count > 0) { seen.add(key); results.push({ type: "city+style", city: citySecond, style, count }); }
            }
          }
        }
      }
    ...
    const matchedCities = allCities
      .filter((c) => expandedTokens.some(t => normalizePolish(c).includes(t)))
      .slice(0, 4);
    ...
    for (const style of matchedStyles) {
      results.push({ type: "style", style, count: styleCounts[style] ?? 0 });
    }

    // ── Schools (diacritic-forgiving + synonym-aware) ──
    const matchedSchools = listings
      .filter((l) => {
        const haystack = normalizePolish(l.name) + ' ' + normalizePolish(l.address) + ' ' + normalizePolish(l.city) + ' ' + normalizePolish(l.neighborhood ?? '') + ' ' + l.styles.map(s => normalizePolish(s)).join(' ');
  ...
  const activeResults = $derived(query.trim().length > 0 ? combinedResults : topCities);
    ...
    try {
      ...
      if (tokens.length >= 2) {
        const findCity = (part: string) =>
          ...
          ?? (part.length >= 3 ? allCities.find(c => normalizePolish(c).startsWith(part)) : undefined);
        // Check if any token group matches a city (prefix or suffix)
        for (let i = 1; i <= tokens.length - 1; i++) {
          const city = findCity(tokens.slice(0, i).join(' ')) || findCity(tokens.slice(i).join(' '));
          if (city) { params.set('city', city); break; }
        }
      }
    ...
    try {
      ...
      if (result?.latitude != null && result?.longitude != null) {
        ...
        for (const [city, coords] of cityCentroids) {
          const d = haversine(result.latitude, result.longitude, coords.lat, coords.lng);
          if (d < minDist) { minDist = d; nearest = city; }
        }
        if (nearest) {
          const label = description || query.trim();
          goto(`/${nearest.toLowerCase()}?lat=${result.latitude}&lng=${result.longitude}&label=${encodeURIComponent(label)}`);
        }
      }
    } finally {
  ...
  function navigateToResult(result: SearchResult) {
    ...
    if (result.type === "city") {
      ...
      goto(`/category/${result.style.toLowerCase().replace(/\s+/g, "-")}`);
    } else if (result.type === "city+style") {
      goto(`/${result.city.toLowerCase()}?style=${encodeURIComponent(result.style)}`);
    }
    query = "";
    showDropdown = false;
  }
  ...
  async function navigatePostal(code: string) {
    ...
    try {
      ...
      if (result?.latitude != null && result?.longitude != null) {
        ...
        if (nearest) {
          const label = result.locationName ? `${code} ${result.locationName}` : code;
          goto(`/${nearest.toLowerCase()}?lat=${result.latitude}&lng=${result.longitude}&label=${encodeURIComponent(label)}`);
        }
      }
    } finally {
  ...
  function handleKeydown(e: KeyboardEvent) {
    ...
    if (e.key === "Tab" && results.length > 0 && showDropdown) {
      ...
      if (activeIndex >= 0 && activeIndex < results.length) {
        ...
        const action = resolveSearch(query.trim(), { page: 'main' } as SearchContext, data.lookups);
        ...
        switch (action.action) {
          case 'route_to_city': {
            const filter = action.styleFilter ? `?style=${encodeURIComponent(action.styleFilter)}` : '';
            goto(`/${cityNameFromSlug(action.citySlug)}${filter}`);
            query = ""; showDropdown = false;
            break;
          }
          case 'route_to_style':
          ...
          default: {
            ...
            const qn = normalizePolish(query.trim());
            const fuzzyCity = allCities.find((c) => normalizePolish(c).startsWith(qn));
            if (fuzzyCity) {
  ...
  const cityCentroids = $derived.by(() => {
    ...
    for (const city of allCities) {
      const schools = listings.filter(
        (s) => s.city === city && s.latitude != null && s.longitude != null,
      );
      if (schools.length === 0) continue;
      const lat =
        schools.reduce((sum, s) => sum + s.latitude!, 0) / schools.length;
      const lng =
        schools.reduce((sum, s) => sum + s.longitude!, 0) / schools.length;
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        if (nearest) {
          ...
          try {
            const res = await fetch(`/api/geocode?revLat=${userLat}&revLng=${userLng}`);
            const result = await res.json();
            if (result?.locationName) label = result.locationName;
          } catch {}
          goto(`/${nearest.toLowerCase()}?lat=${userLat}&lng=${userLng}&label=${encodeURIComponent(label)}`);
        }
      },
      () => {
        locating = false;
  ...
  const majorCities = $derived(
    allCities.slice(0, CITY_PILL_LIMIT)
  );
  const minorCities = $derived(
    allCities.slice(CITY_PILL_LIMIT)
  );
  const visibleCities = $derived(
    showAllCities ? allCities : majorCities,
  );
...
<svelte:head>
  ...
  <meta
    name="description"
    content="Katalog szkół jogi w Polsce. Bez reklam — znajdź szkołę w swoim mieście."
  />
  <meta
    property="og:title"
...
<section class="sf-hero">
  <section class="sf-hero-inner">
    ...
    <p class="sf-hero-sub">
      {listings.length} {pluralSzkola(listings.length)} w {allCities.length} miastach. Bez reklam.
    </p>

    <!-- Unified Search Box -->
    <div class="sf-hero-search-wrapper">
      <div class="sf-hero-search-box" class:sf-search-open={showDropdown && activeResults.length > 0}>
        <div class="search-icon-wrap">
          {#if placesLoading}
            <svg class="search-icon-spin" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>
          {:else}
        ...
        <LocateButton locating={locating} onclick={requestLocation} />

        {#if showDropdown && activeResults.length > 0}
          <div class="search-dropdown" role="listbox">
            {#if query.trim().length === 0}
              <div class="dropdown-group-label">Popularne miasta</div>
            {/if}
            {#each activeResults as result, i (result.type === "school" ? result.id : result.type === "city" ? result.city : result.type === "city+style" ? `${result.city}+${result.style}` : result.type === "postal" ? result.code : result.type === "google-place" ? result.placeId : result.style)}
              {@const isFirst = query.trim().length > 0 && (i === 0 || activeResults[i - 1].type !== result.type)}
              {#if isFirst}
                <div class="dropdown-group-label">{resultLabel(result.type)}</div>
              {/if}
              <button
                class="dropdown-item"
              ...
              >
                {#if result.type === "postal"}
                  ...
                  <span class="dropdown-item-text">{result.code}</span>
                  <span class="dropdown-item-meta">{result.code.length === 6 ? "szukaj w pobliżu" : "wpisz kod XX-XXX"}</span>
                {:else if result.type === "city+style"}
                  <span class="dropdown-item-icon">
                  ...
                  <span class="dropdown-item-text">{result.style} w {result.city}</span>
                  <span class="dropdown-item-meta">{result.count}</span>
                {:else if result.type === "city"}
                  <span class="dropdown-item-icon">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg>
                  </span>
                  <span class="dropdown-item-text">{result.city}</span>
                  <span class="dropdown-item-meta">{result.count} studiów</span>
                {:else if result.type === "style"}
                  <span class="dropdown-item-icon">
                  ...
                  <span class="dropdown-item-text">{result.style}</span>
                  <span class="dropdown-item-meta">{result.count}</span>
                {:else if result.type === "google-place"}
                  <span class="dropdown-item-icon">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg>
                  </span>
                  <span class="dropdown-item-text">{result.description}</span>
                  <span class="dropdown-item-meta">Google Maps</span>
                {:else}
                  <span class="dropdown-item-icon">
                  ...
                  <span class="dropdown-item-text">{result.name}</span>
                  <span class="dropdown-item-meta">{result.city}{result.rating != null ? ` · ★ ${result.rating.toFixed(1)}` : ""}</span>
                {/if}
              </button>
            {/each}
            {#if placeSuggestions.length > 0}
              <div class="dropdown-attribution">Powered by Google</div>
            {/if}
          </div>
        {/if}
      </div>
  ...
  <section class="sf-styles-section">
    ...
    <div class="sf-styles-flex">
      {#each visibleStyles as { style, count } (style)}
        <a href="/category/{style.toLowerCase().replace(/\s+/g, '-')}" class="sf-style-pill">
...
<style>
  ...
  @media (min-height: 900px) {
    .sf-hero {
      padding: clamp(40px, 6vh, 80px) var(--sf-gutter) clamp(40px, 5vh, 64px);
    }
    .sf-hero-tag {
      margin-bottom: 20px;
    }
  ...
  @media (min-height: 1100px) {
    .sf-hero {
      padding: clamp(56px, 7vh, 100px) var(--sf-gutter) clamp(48px, 6vh, 80px);
    }
    .sf-hero-tag {
      font-size: 0.78rem;
      margin-bottom: 28px;
  ...
  @media (min-height: 1400px) {
    .sf-hero {
      padding: clamp(72px, 8vh, 120px) var(--sf-gutter) clamp(64px, 7vh, 100px);
    }
    .sf-hero-tag {
      font-size: 0.82rem;
      margin-bottom: 32px;
