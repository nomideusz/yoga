<script lang="ts">
    ...
    const sortedSchools = $derived.by(
        (): Array<ListingCard & { distance?: number }> => {
            ...
            // Default order: dedicated yoga schools, then gym-named places
            // with yoga, then pilates/meditation; quality (rating weighted
            // by review count) within each tier, alphabetical as tiebreak.
            // Without the gym tier, review-heavy fitness clubs top the list.
            const quality = (s: ListingCard) =>
                (s.rating ?? 0) * Math.log1p(s.reviews ?? 0);
            return [...withDist].sort((a, b) => {
                const t = yogaTier(a.name, a.styles) - yogaTier(b.name, b.styles);
                if (t !== 0) return t;
