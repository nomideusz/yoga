...
export async function search(db: Client, params: SearchParams): Promise<SearchResponse> {
  ...
  let ftsResults = await ftsSearch(db, ftsQuery, citySlug, styleSlug, limit * 3);
...
async function geoOnlySearch(
  ...
  const result = await db.execute({
    sql: `SELECT * FROM schools WHERE lat BETWEEN ? AND ? AND lng BETWEEN ? AND ? LIMIT ?`,
...
function scoreResults(
  ...
  return rows.map(row => {
    ...
    return {
      ...
      city: row.city, citySlug: row.city_slug,
      postcode: row.postcode, lat: row.lat, lng: row.lng,
      phone: row.phone, website: row.website,
      distanceKm, walkingMin, score,
    };
  });
}
...
export async function autocomplete(
  ...
  if (context.page === 'city' && context.citySlug) {
    // City page: suggest districts, styles, schools IN this city only
...
async function addGlobalSchoolSuggestions(db: Client, n: string, out: AutocompleteResult[]) {
  const r = await db.execute({
    sql: 'SELECT slug, name, city FROM schools WHERE name_n LIKE ? LIMIT 4',
    args: [`${n}%`],
  });
  for (const row of r.rows as any[]) {
...
async function addDistrictSuggestions(db: Client, n: string, citySlug: string, out: AutocompleteResult[]) {
  const r = await db.execute({
    sql: 'SELECT DISTINCT district FROM schools WHERE city_slug = ? AND district_n LIKE ? LIMIT 4',
    args: [citySlug, `${n}%`],
  });
  for (const row of r.rows as any[]) {
    if (row.district) out.push({ text: row.district, type: 'district' });
  }
}

async function addStyleSuggestionsInCity(db: Client, n: string, citySlug: string, out: AutocompleteResult[]) {
  const r = await db.execute({
    sql: `SELECT DISTINCT value AS style FROM schools, json_each(schools.styles)
          WHERE city_slug = ? AND LOWER(value) LIKE ? LIMIT 4`,
    args: [citySlug, `${n}%`],
  });
  for (const row of r.rows as any[]) {
    out.push({ text: row.style, type: 'style' });
  }
}

async function addSchoolSuggestionsInCity(db: Client, n: string, citySlug: string, out: AutocompleteResult[]) {
  const r = await db.execute({
    sql: 'SELECT slug, name FROM schools WHERE city_slug = ? AND name_n LIKE ? LIMIT 4',
    args: [citySlug, `${n}%`],
  });
  for (const row of r.rows as any[]) {
    out.push({ text: row.name, type: 'school', slug: row.slug });
  }
}
